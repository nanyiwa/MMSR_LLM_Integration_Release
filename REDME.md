- # 🌟 EmoChat Pro: 基于多模态融合与大语言模型的情感交互系统
  ...
  * **硬件**: 推荐使用配备 NVIDIA GPU (支持 CUDA) 或 Apple Silicon (Mac M系列芯片) 的设备以加速推理。无 GPU 环境亦可运行，但推理延迟会增加。
  ...
  ```bash
  git clone https://github.com/nanyiwa/MMSR_LLM_Integration_Dev.git
  cd MMSR_LLM_Integration_Dev
  ```
  
  ### 3. 下载模型权重
  
  由于模型文件（RoBERTa-Large、ACRNN_v3 等）体积较大，未直接包含在源码仓库中。
  
  1. 请前往本项目的 Releases 页面下载最新发布的权重压缩包（ `weights.zip`）。
  
  2. 将压缩包解压到项目的**根目录**下。
  
  3. 请严格检查解压后的文件结构，确保 `weights` 文件夹与 `src`、`backend` 同级：
  
     ```Plaintext
     MMSR_LLM_Integration_Dev/
     ├── backend/
     ├── frontend/
     ├── src/
     └── weights/          <-- 必须放在这里！
         ├── audio/
         ├── text/         <-- 包含 config.json, model.safetensors 等
         ├── vision/
         └── fusion/
     ```
  ...
  启动成功后，浏览器访问终端中输出的本地链接（通常为 `http://localhost:5173`），即可开始体验 EmoChat Pro 多模态情感交互！
